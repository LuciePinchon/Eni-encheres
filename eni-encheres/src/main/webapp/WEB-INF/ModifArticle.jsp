...
<c:if test="${not empty listeCodesErreur}">
	...
	<div>
		<form action="${pageContext.request.contextPath}/modifArticle?idArticle=${articleAModifier.getNoArticle()}" method="post" class="card">	
			<section class="venteArticle">
				<label for="nouvelArticle">Article</label>
				<input type="text" id="nouvelArticle" name="nouvelArticle" required value="${articleAModifier.getNomArticle()}">
				<!-- TODO : ajouter value dans les champs input pour récupérer les valeurs de la vente -->
				<!-- cela servira en cas de modification du formulaire -->
			
				<label for="descriptionArticle">Description</label>
				<textarea id="descriptionArticle" name="descriptionArticle" required>${articleAModifier.getDescriptionArticle()}</textarea>
			
				<c:set var="noCategorie" value="${articleAModifier.getCategorieArticle()}"/>
				...
				<input type="number" name="miseAPrix" id="miseAPrix" value="${articleAModifier.getPrixInitial()}" required>
			
				<label for="debutEnchere">Début de l'enchère</label>
				<input type="date" name="debutEnchere" id="debutEnchere" value="${articleAModifier.getDateDebutEnchere()}" required>
			
				<label for="finEnchere">Fin de l'enchère</label>
				<input type="date" name="finEnchere" id="finEnchere" value="${articleAModifier.getDateFinEnchere()}" required>
			...
			<p>Retrait</p>
				...
				<input type="submit" class="button-a" name="action" value="Modifier" class="button">
				
				<a href="/eni-encheres/pageAccueil" class="button-a"><input type="button" value="Annuler" class="button"></a>
			
				<input type="submit" class="button-a" name="action" value="Supprimer" class="button">
			
			<!--  
			<c:if test="${dateActuelle.isBefore(articleAModifier.getDateDebutEnchere())}">
				<p>Quelque chose ne va pas avec tes boutons jeune fille.</p> 
			</c:if> 
			-->
